...
VS Code 에서는 `.vscode/tasks.json` 이 작업 메뉴도 제공합니다.
**Ctrl + Shift + P → `Tasks: Run Task`** 로 지금 연 파일을 볼트에 바로 저장할 수 있습니다
(Claude 없이 동작하며, 요약 없이 파일 그대로 들어갑니다). 다른 폴더에서도 쓰려면
`Tasks: Open User Tasks` 에 같은 내용을 넣으세요.
